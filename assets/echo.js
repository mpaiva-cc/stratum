...
`;
  ...
  // ── text extraction ────────────────────────────────────────
  // The readable-block selector — shared by extractChunks (page reading) and the
  // double-click handler (map a clicked block back to its chunk).
  const READABLE_SELECTOR = 'h1, h2, h3, h4, h5, h6, p, li, blockquote, .lede, .section-h, .section-dek, .rule-body, .answer-headline, .r-blurb';
  function extractChunks() {
    ...
    const blocks = root.querySelectorAll(READABLE_SELECTOR);
    const chunks = [];
    blocks.forEach(el => {
  ...
  function indexForElement(target) {
    ...
    if (!block) return -1;
    let idx = state.chunks.findIndex(c => c.el === block);
    if (idx < 0) {
      // deduped out, or DOM changed since last extract — re-extract once and retry
      state.chunks = extractChunks();
      state.batches = [];
      idx = state.chunks.findIndex(c => c.el === block);
    }
    if (idx < 0) idx = state.chunks.findIndex(c => c.el.contains(target));
    return idx;
  }
  ...
  document.addEventListener('dblclick', (e) => {
    // leave interactive, editable, and Echo's own UI alone
    if (e.target.closest('a, button, input, textarea, select, label, summary, [contenteditable], .echo-panel, .echo-btn')) return;
    const idx = indexForElement(e.target);
