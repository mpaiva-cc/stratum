...
test('persona scope authority matches the design matrix', () => {
  assert.deepStrictEqual(FR.ROLES.chro.scopes.slice().sort(), SCOPES.slice().sort());
  assert.deepStrictEqual(FR.ROLES.hrbp.scopes.slice().sort(), SCOPES.slice().sort());
  ...
  assert.deepStrictEqual(FR.ROLES.ic.scopes.slice().sort(), SCOPES.slice().sort());
